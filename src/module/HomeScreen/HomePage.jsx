...
import { transactions } from "../../constants/Colors";
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-5 pt-5 pb-24">
            ...
            <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
              {friends.map((friend) => (
                <div
                  ...
                  className="flex flex-col items-center gap-2 shrink-0 group active:scale-95 transition cursor-pointer"
                >
                  <div className="relative">
                    <div
                      className="w-14 h-14 rounded-full flex items-center justify-center text-base font-bold shadow-md ring-2 ring-white transition-transform group-hover:scale-105"
                      style={{
                    ...
                    <div
                      className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-white ${friend.direction === "owes" ? "bg-orange-400" : "bg-emerald-400"}`}
                    ></div>
                  </div>
                  ...
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full inline-flex items-center gap-0.5 ${
                      friend.direction === "owes"
                        ? "bg-orange-100 text-orange-600"
                        : "bg-emerald-100 text-emerald-600"
                    }`}
                  >
                    {!String(friend.balance).includes("-") && (
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="text-xs font-medium text-indigo-500">
              See all →
            </button>
          </div>
          <div className="space-y-2">
            {transactions.map((txn) => (
              <div
                key={txn.id}
                className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 active:bg-gray-50 transition flex items-center gap-3"
              >
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shadow-sm flex-shrink-0"
                  style={{
                    backgroundColor: txn.friend.bgColor,
                    color: txn.friend.textColor,
                  }}
                >
                  {txn.friend.initials}
                </div>
                ...
                <div
                  className={`text-base font-bold ${txn.type === "give" ? "text-orange-500" : "text-emerald-500"}`}
                >
