...
const FriendDetailPage = () => {
  ...
  const { data: transactions = [], isLoading: txnsLoading } = useTransactions(id);
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 flex flex-col overflow-hidden">
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-white/20 px-5 pt-6 pb-5 flex-shrink-0 shadow-sm">
        ...
        >
          <span className="text-sm text-gray-600">
            {direction === "owes" ? `${friend.name} owes you` : `You owe ${friend.name}`}
          </span>
          <span className={`text-lg font-bold ${direction === "owes" ? "text-orange-600" : "text-emerald-600"}`}>
        ...
        <div className="flex gap-2 mt-3">
          <button
            onClick={() => navigate(`/friends/${friend.id}/add-transaction?type=gave`)}
            className="flex-1 text-xs font-semibold text-white bg-orange-500 py-2 rounded-lg active:scale-95 transition"
          >
            You Gave
          </button>
          <button
            onClick={() => navigate(`/friends/${friend.id}/add-transaction?type=received`)}
            className="flex-1 text-xs font-semibold text-white bg-emerald-500 py-2 rounded-lg active:scale-95 transition"
          >
            You Got
          </button>
              ...
              <div key={tx.id} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex items-center gap-3">
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-sm flex-shrink-0 ${
                    tx.transaction_type === "gave" ? "bg-orange-100 text-orange-600" : "bg-emerald-100 text-emerald-600"
                  }`}
                >
                  {tx.transaction_type === "gave" ? "↑" : "↓"}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-800 truncate">{tx.description || "No description"}</p>
                ...
                <span className={`text-sm font-bold ${tx.transaction_type === "gave" ? "text-orange-600" : "text-emerald-600"}`}>
